...
########################### Parameters
#name of file
NW.file <- "data/NEOWISE_latest.csv"
# bin width in days
bin.width = 1 # days
plot.w1 = FALSE
w1.col = "blue"
...
if(!is.na(earliest.mjd)) {
	tmin <- max(tmin,earliest.mjd)
}
...
# dip mask for dip times
source("input_files/dip_mask.R")
mjd.dip.starts <- JD2MJD(as.numeric(dip.mask$JD.begins))
line.color <- "red"
...
title(plot.title)
verticalDateLines(mjd.dip.starts,dip.mask$dipname, y.limits, line.color)
grid(col="black")
...
verticalDateLines(mjd.dip.starts,dip.mask$dipname,c(-0.05,0.05), line.color)
